function results = run_affine_occlude_3d(data_name)
...
for iter = 1:num_of_scales
    ...
    s = 1 + ((iter - 1) * scale);
    [data.img1] = random_img_per_slice(tdata, width * s, height * s);
    create_gif(data.img1, sprintf('%dx%dx%d (%.2f).gif', width,  height,  1, s));
